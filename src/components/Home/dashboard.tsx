...
export default function Dashboard({ tools }: props) {
  ...
  const brandCounts = tools.reduce(
    (acc: Record<string, number>, tool: ToolData) => {
  ...
  const divisionCounts = tools.reduce(
    (acc: Record<string, number>, tool: ToolData) => {
  ...
  const totalItems = brandChartData.reduce(
    (acc: number, curr) => acc + curr.visitors,
    0
  );
  ...
  const chartConfig = {
    ...
    chrome: {
      label: brandChartData[0]?.browser || "Brand 1",
      color: "hsl(var(--chart-1))",
    },
    safari: {
      label: brandChartData[1]?.browser || "Brand 2",
      color: "hsl(var(--chart-2))",
    },
    firefox: {
      label: brandChartData[2]?.browser || "Brand 3",
      color: "hsl(var(--chart-3))",
    },
    edge: {
      label: brandChartData[3]?.browser || "Brand 4",
      color: "hsl(var(--chart-4))",
    },
    other: {
      label: brandChartData[4]?.browser || "Other",
    ...
    <div>
      ...
      <div className="w-full my-7 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* Pie Chart Card */}
          <Card className="flex justify-start flex-col">
            <CardHeader className="items-start justify-start flex pb-0">
          ...
          <Card className="flex flex-col">
            <CardHeader className="items-start justify-start flex pb-10">
            ...
            <CardFooter className="flex items-center justify-between text-sm">
              <span>
                Division with most tools: {divisionChartData[0]?.month}
              </span>
            </CardFooter>
          </Card>
        </div>
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold mb-4">
            Tool Analytics{" "}
            <span className="text-sm text-slate-400">(using AI)</span>
          </h2>
